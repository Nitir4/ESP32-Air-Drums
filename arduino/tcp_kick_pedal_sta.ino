/*
 * tcp_kick_pedal_sta.ino — ESP32 Kick Pedal (TCP Station Mode)
 * -------------------------------------------------------------
 * ESP32 WROVER KIT acting as WiFi Station (STA)
 * Connects to AP (ESPDRUMS_1) and forwards MIDI to it via TCP
 * 
 * Architecture:
 *   KICK (WROVER) → [WiFi] → AP (ESPDRUMS_1) → [WiFi] → Desktop
 * 
 * Hardware:
 *   - ESP32 WROVER KIT
...
const char* AP_SERVER_IP = "192.168.4.1";  // AP's IP address
const int AP_SERVER_PORT = 5000;           // AP's relay port
...
void setup() {
  ...
  delay(100);
  Serial.println("\n========================================");
  Serial.println("ESP32 KICK PEDAL - TCP Station Mode");
  Serial.println("========================================");
...
void loop() {
  ...
  if (!pedal_pressed) {
    if (az_f < THRESHOLD_PRESS && (now - last_trigger) > DEBOUNCE_MS) {
      ...
      Serial.printf("✅ Kick Note On sent to AP (note %d)\n", MIDI_NOTE);
