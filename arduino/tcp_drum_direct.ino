/*
 * tcp_drum_direct.ino — ESP32 Air Drum (Direct to Desktop - Debug)
 * -----------------------------------------------------------------
 * ESP32 XIAO C3 connects to ESPDRUMS AP but sends MIDI DIRECTLY to desktop
 * Bypasses AP relay - sends MIDI straight to desktop TCP server on port 6000
 * 
 * Architecture:
 *   DRUM (XIAO C3) → [ESPDRUMS WiFi] → Desktop App (port 6000)
 *   (AP Relay is bypassed - no forwarding through port 5000)
 * 
 * Use this for:
 *   - Debugging drum hit detection
 *   - Testing without AP relay overhead
 *   - Isolating drum pad issues
 * 
 * Hardware:
 *   - ESP32 XIAO C3
...
void setup() {
  ...
  delay(100);
  Serial.println("\n========================================");
  Serial.println("AIR DRUM - DIRECT DEBUG MODE");
  Serial.println("========================================");
  ...
  Serial.println("\n🥁 Ready - swing to trigger drum hit!\n");
}
...
void loop() {
  ...
  if (DEBUG_SENSOR && tcpClient.connected()) {
    debug_counter++;
    if (debug_counter >= 20) {
      Serial.printf("az_f: %.3f | Threshold: %.2f | TCP: OK\n",
                    az_f, THRESHOLD_HIT);
      debug_counter = 0;
    }
  }
