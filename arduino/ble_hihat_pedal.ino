/*
 * ble_hihat_pedal.ino — ESP32 Hi-Hat Pedal (Arduino BLE)
 * --------------------------------------------------------
...
// Configuration
// ========================================
#define BLE_NAME "ESPDRUMS_HIHAT"

// MIDI Settings - Hi-Hat has TWO sounds!
#define MIDI_NOTE_CLOSED 42   // Closed Hi-Hat (foot down)
#define MIDI_NOTE_OPEN 46     // Open Hi-Hat (foot up)
...
#define THRESHOLD_PRESS -0.6  // Foot pressing down
...
// State Machine
bool pedal_closed = false; // False=OPEN, True=CLOSED
...
void setup() {
  ...
  delay(100);
  Serial.println("\n========================================");
  Serial.println("ESP32 Hi-Hat Pedal - BLE MIDI");
  Serial.println("========================================");
  ...
  // Calibration - assumes OPEN position (foot up)
  Serial.println("Calibrating - KEEP FOOT OFF PEDAL (OPEN position)...");
...
void loop() {
  ...
  if (!pedal_closed) {
    ...
    if (az_f < THRESHOLD_PRESS && (now - last_trigger) > DEBOUNCE_MS) {
      ...
      Serial.printf("✅ Closed Hi-Hat Note On sent (note %d)\n", MIDI_NOTE_CLOSED);
    ...
    if (az_f > THRESHOLD_RELEASE && (now - last_trigger) > DEBOUNCE_MS) {
      ...
      Serial.printf("✅ Open Hi-Hat Note On sent (note %d)\n", MIDI_NOTE_OPEN);
